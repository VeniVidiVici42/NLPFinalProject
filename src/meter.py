...
def get_auc(id_to_tfidf, annotations):
	actual = []
	expected = []
	
	for annotation in annotations:
		(id, candidates, labels) = annotation
		good_features = id_to_tfidf[id]
		for cand in candidates:
			cand_features = id_to_tfidf[cand]
			score = models.cosine_sim(good_features, cand_features, dim=0)
			actual.append(score)
